...
import { Loader2, ShieldAlert, CheckCircle, RefreshCw, Users, Mail, Trash2, ChevronLeft, ChevronRight, Bell, DatabaseZap, CreditCard, AlertCircle as AlertCircleIcon, KeySquare, User, CalendarDays, Clock, Target, Award, FileWarning, ArrowLeft } from 'lucide-react';
import type { UserProfile } from '@/types';
import { fetchAllActiveUsers, deleteUserAndTheirData, resetUserStatistics, updateUserSubscription } from '@/actions/user-data-actions';
...
export default function ActiveUsersPage() {
  ...
  const fetchUsers = useCallback(async (page: number, callerUid: string) => {
    ...
    setFetchError(null);
    try {
      const result = await fetchAllActiveUsers(page, USERS_PER_PAGE, callerUid);
      if (result.success && result.users) {
        setActiveUsers(result.users as UserProfile[]);
        setTotalCount(result.totalCount || 0);
      } else {
        throw new Error(result.error || t('admin.activeUsers.errorFetchingUserList'));
      }
    } catch (error: any) {
    ...
    <>
      <div className="container mx-auto py-8">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-3"><Users className="h-8 w-8 text-primary" /><h1 className="text-3xl font-bold">{t('admin.activeUsers.title')}</h1></div>
          <Button variant="outline" onClick={() => currentUser && fetchUsers(currentPage, currentUser.uid)} disabled={isLoading}><RefreshCw className={`mr-2 h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />{t('admin.activeUsers.refreshButton')}</Button>
        </div>
        <p className="text-muted-foreground mb-10">{t('admin.activeUsers.description')}</p>

        {isLoading && <div className="flex justify-center items-center py-10"><Loader2 className="h-10 w-10 animate-spin text-primary" /><p className="ml-3 text-muted-foreground">{t('admin.activeUsers.loadingStats')}</p></div>}
        {!isLoading && fetchError && <Alert variant="destructive"><AlertCircleIcon className="h-4 w-4" /><AlertTitle>{t('toast.errorTitle')}</AlertTitle><AlertDescription>{fetchError}</AlertDescription></Alert>}
        {!isLoading && !fetchError && activeUsers.length === 0 && <Alert><CheckCircle className="h-4 w-4" /><AlertTitle>{t('admin.activeUsers.noUsersTitle')}</AlertTitle><AlertDescription>{t('admin.activeUsers.noUsersDesc')}</AlertDescription></Alert>}
