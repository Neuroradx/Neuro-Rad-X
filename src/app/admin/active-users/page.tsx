      ...
      <div className="container mx-auto py-8 max-w-6xl">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">{t('admin.activeUsers.title')}</h1>
              <p className="text-sm text-muted-foreground mt-0.5">{t('admin.activeUsers.description')}</p>
            </div>
          </div>
          <Button variant="outline" size="sm" className="rounded-lg shrink-0" onClick={() => currentUser && fetchUsers(currentPage, currentUser.uid)} disabled={isLoading}>
        ...
        {isLoading && (
          <div className="flex flex-col justify-center items-center py-16 rounded-xl border border-border/60 bg-muted/20">
            ...
            <p className="mt-3 text-sm text-muted-foreground">{t('admin.activeUsers.loadingStats')}</p>
          </div>
        )}
        ...
        {!isLoading && !fetchError && activeUsers.length === 0 && (
          <Alert className="rounded-xl border-border/60">
            ...
            <AlertTitle>{t('admin.activeUsers.noUsersTitle')}</AlertTitle>
            <AlertDescription>{t('admin.activeUsers.noUsersDesc')}</AlertDescription>
          </Alert>
        )}
